...
class ColumnSizer {
  ...
  void _refresh(double availableWidth) {
    ...
    if (dataGridConfiguration.columnWidthMode != ColumnWidthMode.none || hasAnySizerColumn) {
  ...
  GridColumn? _getColumnToFill(DataGridConfiguration dataGridConfiguration) {
    ...
    if (column != null) {
      ...
      if (dataGridConfiguration.columnWidthMode == ColumnWidthMode.lastColumnFill) {
        final GridColumn? lastColumn = dataGridConfiguration.columns.lastWhereOrNull(
          (GridColumn c) => c.visible && c.width.isNaN,
        );
        if (lastColumn == null) {
          return null;
        }
    ...
    if (remainingColumnWidths > 0 &&
        ...
        (totalColumnSize != 0 ||
            ...
            (dataGridConfiguration.columns.any(
                  (GridColumn col) => col.columnWidthMode == ColumnWidthMode.fill,
                ) ||
  ...
  Object? _getCellValue(DataGridRow dataGridRow, GridColumn column) {
    final firstCell = dataGridRow.getCells().firstWhereOrNull(
  ...
  void _setFillWidth(
    ...
    while (columns.isNotEmpty) {
      isRemoved = false;
      removedWidth = 0;
      final double fillWidth = (totalRemainingFillValue / columns.length).floorToDouble();
      final GridColumn column = columns.first;
      if (column == _autoFillColumn &&
          (column.columnWidthMode == ColumnWidthMode.lastColumnFill ||
              dataGridConfiguration.columnWidthMode == ColumnWidthMode.lastColumnFill)) {
...
class ColumnResizeController {
  ...
  final ValueNotifier<bool> canSwitchResizeColumnCursorNotifier = ValueNotifier(true);
  ...
  void _onResizingStart() {
    if (_resizingLine != null && !isResizeIndicatorVisible) {
      ...
      if (dataGridConfiguration.source.groupedColumns.isNotEmpty) {
        ...
        _currentResizingColumn = dataGridConfiguration.columns[_resizingLine!.lineIndex];
      }
  ...
  bool _canAllowResizing(double downX, DataCellBase? dataColumn, {bool canAllowBuffer = true}) {
    if (dataColumn != null &&
      ...
      final DataCellBase? dataCell = dataColumn.dataRow!.visibleColumns.firstWhereOrNull((
      ...
      if (dataCell != null) {
        ...
        final double origin = isLTR ? _resizingLine!.clippedOrigin : _resizingLine!.corner;
        final double corner = isLTR ? _resizingLine!.corner : _resizingLine!.clippedOrigin;

        if (canAllowBuffer) {
          if ((cellLeft == lineIndex && (corner - downX).abs() <= _hitTestPrecision!) ||
              (cellRight == lineIndex && (origin - downX).abs() <= _hitTestPrecision!) ||
              (lineIndex > cellLeft && lineIndex < cellRight)) {
            return false;
          }
        } else {
  ...
  double getXPosition(DataGridConfiguration dataGridConfiguration, double localPosition) {
    ...
    if (dataGridConfiguration.textDirection == TextDirection.ltr) {
      ...
      return localPosition - (scrollController.position.maxScrollExtent - scrollController.offset);
    }
  }
  ...
  void setDataCell(DataCellBase dataCell) {
    if (_isHeaderRow(dataCell.dataRow!)) {
      if (dataGridStateDetails().allowColumnsResizing && !isResizing && !isResizeIndicatorVisible) {
  ...
  void onPointerMove(PointerMoveEvent event, DataRowBase dataRow) {
    if (_isHeaderRow(dataRow)) {
      ...
      if (_resizingLine != null && isResizeIndicatorVisible) {
        ...
        _resizingColumnWidth += isLTR ? event.localDelta.dx : -event.localDelta.dx;
  ...
  void onPointerUp(PointerUpEvent event, DataRowBase dataRow) {
    ...
    if (!_canStartResizeInMobile && isResizeIndicatorVisible) {
      if (dataGridConfiguration.columnResizeMode == ColumnResizeMode.onResizeEnd) {
...
class DataGridFilterHelper {
  ...
  TextStyle get textStyle => _dataGridStateDetails().dataGridThemeHelper!.filterPopupTextStyle!;
    ...
    if (_unCheckedItemsCount == 0 &&
      ...
      for (final FilterElement value in checkboxFilterHelper.items) {
        if (value.isSelected == useSelected) {
          final FilterType filterType = useSelected ? FilterType.equals : FilterType.notEqual;
          FilterOperator filterOperator = useSelected ? FilterOperator.or : FilterOperator.and;
          final String? filterValue = value.value == '(Blanks)' ? null : value.value.toString();
      ...
      switch (advancedFilterHelper.advancedFilterType) {
        ...
        case AdvancedFilterType.date:
          if (value is! DateTime) {
            ...
            if (values.length > 2 && values.every((String element) => element.isNotEmpty)) {
  ...
  void _debugCheckDataType(DataGridConfiguration dataGridConfiguration) {
    ...
    if (source.effectiveRows.isNotEmpty && source.filterConditions.isNotEmpty) {
      for (final String columnName in source.filterConditions.keys) {
        final GridColumn? column = dataGridConfiguration.columns.firstWhereOrNull(
  ...
  void _applyViewFilter(GridColumn column) {
    ...
    final DataGridConfiguration dataGridConfiguration = _dataGridStateDetails();
    if (source.filterConditions.containsKey(column.columnName)) {
      final List<FilterCondition>? filterConditions = source.filterConditions[column.columnName];
      ...
      if (_previousDataRows.isNotEmpty) {
        filteredRows = _getFilterRows(_previousDataRows, <String, List<FilterCondition>>{
  ...
  List<DataGridRow> _getPreviousFilteredRows(String columnName) {
    ...
    final DataGridSource source = _dataGridStateDetails().source;
    final List<FilterCondition>? conditions = source.filterConditions[columnName];
      ...
      if (cell != null) {
        if (cell.value != null) {
          ...
          if (cell.value is Map) {
            ...
            final bool alreadyExists = cellValues.any(
              (existing) =>
  ...
  Future<void> _loadInitialPaginatedData(
    GridColumn column,
    List<FilterElement> localItems, {
...
class DataGridCheckboxFilterHelper {
  ...
  PaginatedFilterHelper? paginatedFilterHelper;
    ...
    if (searchText.isEmpty) {
      ...
      if (_previousDataGridSource.isNotEmpty) {
        ...
        for (final FilterElement item in filterCheckboxItems) {
          final FilterElement? filterElement = _previousDataGridSource.firstWhereOrNull(
            (FilterElement i) => item.value == i.value,
          );
  ...
  void initializePaginatedFiltering(
    String columnName,
    int columnIndex,
    PaginatedFilterCallback callback,
  ...
  Future<void> _handlePaginatedSearch(String searchText, {VoidCallback? onCompleted}) async {
    ...
    try {
      ...
      // Set loading state
      _usePaginatedFiltering = true;
      paginatedFilterHelper!.setCurrentSearchText(searchText);
      if (searchText.trim().isNotEmpty) {
        await paginatedFilterHelper!.loadInitialData(searchText: searchText);
        _searchedItems = paginatedFilterHelper!.items;
      ...
      if (searchText.isEmpty) {
        ...
        if (selectedItems.isNotEmpty) {
          for (final FilterElement item in filterCheckboxItems.toList()) {
            final FilterElement? filterElement = selectedItems.firstWhereOrNull(
          ...
          for (final FilterElement item in selectedItems) {
            final FilterElement? filterElement = filterCheckboxItems.firstWhereOrNull(
              (i) => item.value == i.value,
            );
  ...
  Future<void> loadInitialPaginatedData({
    ...
    try {
      ...
      await paginatedFilterHelper!.loadInitialData();
      items = paginatedFilterHelper!.items;
      advancedFilterHelper.items = items;
      filterCheckboxItems = items;
  ...
  Future<void> loadNextPage({
    ...
    try {
      await paginatedFilterHelper!.loadNextPage(selectAll: selectAll);
      if (paginatedFilterHelper!.currentSearchText.trim().isNotEmpty) {
        _searchedItems = paginatedFilterHelper!.items;
        filterCheckboxItems = _searchedItems;
      } else {
        items = paginatedFilterHelper!.items;
      ...
      if ((filterFrom == FilteredFrom.checkboxFilter || filterFrom == FilteredFrom.none) &&
          paginatedFilterHelper!.currentSearchText.trim().isEmpty) {
        onSetPreviousDataGridSource();
      }
  ...
  bool get hasMoreData => paginatedFilterHelper?.hasMoreData ?? false;

  /// Gets whether data is currently being loaded.
  ValueNotifier<bool> get isLoading => paginatedFilterHelper?.isLoading ?? ValueNotifier(false);
  ...
  void disposePaginatedFiltering() {
    ...
    paginatedFilterHelper?.clear();
    paginatedFilterHelper = null;
  }
}
  ...
  void setAdvancedFilterValues(
    ...
    if (filterConditions.isNotEmpty) {
      ...
      if (filterConditions.length == 1) {
        filterType2 = dataGridConfiguration.localizations.equalsDataGridFilteringLabel;
        filterValue2 = null;
    ...
    firstValueTextController.text = dataGridConfiguration.dataGridFilterHelper!.getDisplayValue(
      filterValue1,
    );
    secondValueTextController.text = dataGridConfiguration.dataGridFilterHelper!.getDisplayValue(
      filterValue2,
    );
  }
...
class ColumnDragAndDropController {
  ...
  Future<void> _autoScrollIfNecessary(
    ...
    if (!autoScrolling && !disableScrolling) {
      ...
      final double scrollEnd = scrollOrigin!.dx + dataGridConfiguration.viewWidth - dragThreshold;
      ...
      if (newOffset != null && (newOffset - position.pixels).abs() >= 1.0) {
        ...
        dataGridConfiguration.container.scrollColumns
          ...
          ..updateScrollbar();
        autoScrolling = false;
        columnIndex = getColumnLineInfo(dataGridConfiguration, details.position.dx)?.lineIndex;
        ...
        if (rowIndex == headerIndex &&
          ...
          if (_dragDirectionChangedPosition != null) {
            if (event.delta.dx.sign < 0 &&
                event.position.dx < _dragDirectionChangedPosition! - indicatorPositionThreshold) {
...
typedef PaginatedFilterCallback =
    Future<PaginatedFilterResponse> Function(PaginatedFilterRequest request);
...
class PaginatedFilterHelper {
  ...
  /// The data grid source to check filter conditions.
  ...
  Future<void> _loadPage({bool selectAll = true, DataGridSource? dataGridSource}) async {
    ...
    try {
      ...
      // Helper function to determine if a value should be selected
      bool isSelected(Object? value) {
      ...
      final List<FilterElement> newItems =
          response.values
              .map(
                (Object? value) =>
                    FilterElement(value: value ?? '(Blanks)', isSelected: isSelected(value)),
              )
              .toList();
      _items.clear();
